<script setup lang="ts">
import type { JadwalUjian, PeriodePendaftaran, SesiUjian, RuangUjian } from '~/types/api'

definePageMeta({
  layout: 'admin'
})

const adminApi = useAdminApi()
const toast = useToast()

const isLoading = ref(true)
const jadwalList = ref<JadwalUjian[]>([])
const periodeList = ref<PeriodePendaftaran[]>([])
const sesiList = ref<SesiUjian[]>([])
const ruangList = ref<RuangUjian[]>([])

// Filters
const selectedPeriodeId = ref<number | ''>('')
const activePeriodeId = ref<number | null>(null)

// Modal State
const isModalOpen = ref(false)
const modalMode = ref<'create' | 'edit'>('create')
const isSubmitting = ref(false)
const deleteConfirmationOpen = ref(false)
const itemToDelete = ref<JadwalUjian | null>(null)

// Form Data
const formData = reactive({
  id: 0,
  periode_id: 0,
  tanggal: '',
  sesi_id: 0,
  ruang_id: 0,
  kuota: 20,
  is_active: true
})

const errors = ref<Record<string, string[]>>({})

// Fetch Master Data
const loadMasterData = async () => {
  try {
    const [periodeRes, sesiRes, ruangRes] = await Promise.all([
      adminApi.getPeriodeList(),
      adminApi.getSesiList(),
      adminApi.getRuangList()
    ])

    if (periodeRes.success && periodeRes.data) {
      periodeList.value = periodeRes.data
      // Set default selected periode to active one
      const active = periodeList.value.find(p => p.is_active)
      if (active) {
        selectedPeriodeId.value = active.id
        activePeriodeId.value = active.id
        formData.periode_id = active.id
      } else if (periodeList.value.length > 0 && periodeList.value[0]) {
         // Fallback to first if no active found
         selectedPeriodeId.value = periodeList.value[0].id
         formData.periode_id = periodeList.value[0].id
      }
    }

    if (sesiRes.success && sesiRes.data) {
      sesiList.value = sesiRes.data
    }

    if (ruangRes.success && ruangRes.data) {
      ruangList.value = ruangRes.data
    }
  } catch (error) {
    console.error('Failed to load master data:', error)
    toast.add({ title: 'Error', description: 'Gagal memuat data master', color: 'error' })
  }
}

// Fetch Jadwal
const loadData = async () => {
  isLoading.value = true
  try {
    const res = await adminApi.getJadwalList(selectedPeriodeId.value || undefined)
    if (res.success && res.data) {
      jadwalList.value = res.data
    }
  } catch (error) {
    toast.add({ title: 'Error', description: 'Gagal memuat data jadwal ujian', color: 'error' })
  } finally {
    isLoading.value = false
  }
}

onMounted(async () => {
  await loadMasterData()
  // Data will be loaded via watcher when selectedPeriodeId is set
  // If no periode selected (empty), we load manually
  if (!selectedPeriodeId.value) {
    await loadData()
  }
})

// Watch filter
watch(selectedPeriodeId, () => {
  loadData()
})

// Formatting Helpers
const formatDateDisplay = (dateStr: string) => {
  if (!dateStr) return '-'
  return new Date(dateStr).toLocaleDateString('id-ID', {
    weekday: 'long',
    day: 'numeric',
    month: 'long',
    year: 'numeric'
  })
}

// Actions
const openCreateModal = () => {
  modalMode.value = 'create'
  formData.id = 0
  // Default to filtered periode or active one
  formData.periode_id = (selectedPeriodeId.value as number) || (activePeriodeId.value as number) || 0
  formData.tanggal = new Date().toISOString().slice(0, 10)
  formData.sesi_id = sesiList.value.length > 0 && sesiList.value[0] ? sesiList.value[0].id : 0
  formData.ruang_id = ruangList.value.length > 0 && ruangList.value[0] ? ruangList.value[0].id : 0
  formData.kuota = 20
  formData.is_active = true
  errors.value = {}
  isModalOpen.value = true
}

const openEditModal = (item: JadwalUjian) => {
  modalMode.value = 'edit'
  formData.id = item.id
  formData.periode_id = item.periode_id
  formData.tanggal = item.tanggal ? item.tanggal.slice(0, 10) : ''
  formData.sesi_id = item.sesi_id
  formData.ruang_id = item.ruang_id
  formData.kuota = item.kuota
  formData.is_active = item.is_active
  errors.value = {}
  isModalOpen.value = true
}

const closeModal = () => {
  isModalOpen.value = false
  errors.value = {}
}

const handleSubmit = async () => {
  isSubmitting.value = true
  errors.value = {}
  
  try {
    let res
    const payload = {
      periode_id: formData.periode_id,
      tanggal: formData.tanggal,
      sesi_id: formData.sesi_id,
      ruang_id: formData.ruang_id,
      kuota: formData.kuota,
      is_active: formData.is_active
    }

    if (modalMode.value === 'create') {
      res = await adminApi.createJadwal(payload)
    } else {
      res = await adminApi.updateJadwal(formData.id, payload)
    }

    if (res.success) {
      toast.add({
        title: 'Berhasil',
        description: `Jadwal ujian berhasil ${modalMode.value === 'create' ? 'ditambahkan' : 'diupdate'}`,
        color: 'success'
      })
      await loadData()
      closeModal()
    } else {
      if (res.errors) {
        errors.value = res.errors
      } else {
        toast.add({
          title: 'Gagal',
          description: res.message || 'Terjadi kesalahan saat menyimpan data',
          color: 'error'
        })
      }
    }
  } catch (error: any) {
    console.error('Submit error:', error)
    toast.add({
      title: 'Error',
      description: 'Terjadi kesalahan sistem',
      color: 'error'
    })
  } finally {
    isSubmitting.value = false
  }
}

const confirmDelete = (item: JadwalUjian) => {
  itemToDelete.value = item
  deleteConfirmationOpen.value = true
}

const handleDelete = async () => {
  if (!itemToDelete.value) return
  
  try {
    const res = await adminApi.deleteJadwal(itemToDelete.value.id)
    if (res.success) {
      toast.add({
        title: 'Berhasil',
        description: 'Jadwal ujian berhasil dihapus',
        color: 'success'
      })
      await loadData()
      deleteConfirmationOpen.value = false
      itemToDelete.value = null
    } else {
      toast.add({
        title: 'Gagal',
        description: res.message || 'Gagal menghapus jadwal ujian',
        color: 'error'
      })
    }
  } catch (error) {
    console.error('Delete error:', error)
    toast.add({
      title: 'Error',
      description: 'Terjadi kesalahan saat menghapus data',
      color: 'error'
    })
  }
}
</script>

<template>
  <div class="flex flex-col gap-6">
    <!-- Header -->
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Jadwal Ujian</h1>
        <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">
          Kelola jadwal ujian seleksi masuk
        </p>
      </div>
      <div class="flex gap-3">
        <button 
          @click="openCreateModal"
          class="px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary-600 transition-colors flex items-center gap-2 shadow-sm shadow-primary/30"
        >
          <UIcon name="i-heroicons-plus" class="text-lg" />
          Tambah Jadwal
        </button>
      </div>
    </div>

    <!-- Content -->
    <div class="bg-white dark:bg-surface-dark rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col min-h-[400px]">
      
      <!-- Filters -->
      <div class="p-4 border-b border-slate-200 dark:border-slate-800 flex gap-4">
        <select 
          v-model="selectedPeriodeId"
          class="px-3 py-2 bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 text-slate-700 dark:text-slate-200 min-w-[200px]"
        >
          <option value="">Semua Periode</option>
          <option v-for="periode in periodeList" :key="periode.id" :value="periode.id">
            {{ periode.nama }} {{ periode.is_active ? '(Aktif)' : '' }}
          </option>
        </select>
      </div>

      <!-- Table -->
      <div class="overflow-x-auto">
        <table class="w-full text-left text-sm text-slate-600 dark:text-slate-400">
          <thead class="bg-slate-50 dark:bg-slate-800/50 text-xs uppercase font-semibold text-slate-500">
            <tr>
              <th class="px-6 py-4">Periode</th>
              <th class="px-6 py-4">Waktu Ujian</th>
              <th class="px-6 py-4">Tempat</th>
              <th class="px-6 py-4 text-center">Kuota Peserta</th>
              <th class="px-6 py-4 text-center">Status</th>
              <th class="px-6 py-4 text-right">Aksi</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
            <template v-if="isLoading">
              <tr v-for="i in 3" :key="i">
                <td class="px-6 py-4"><div class="h-4 bg-slate-100 dark:bg-slate-800 rounded animate-pulse w-32"></div></td>
                <td class="px-6 py-4"><div class="h-4 bg-slate-100 dark:bg-slate-800 rounded animate-pulse w-48"></div></td>
                <td class="px-6 py-4"><div class="h-4 bg-slate-100 dark:bg-slate-800 rounded animate-pulse w-32"></div></td>
                <td class="px-6 py-4"><div class="h-4 bg-slate-100 dark:bg-slate-800 rounded animate-pulse w-16 mx-auto"></div></td>
                <td class="px-6 py-4"><div class="h-4 bg-slate-100 dark:bg-slate-800 rounded animate-pulse w-16 mx-auto"></div></td>
                <td class="px-6 py-4"><div class="h-4 bg-slate-100 dark:bg-slate-800 rounded animate-pulse w-16 ml-auto"></div></td>
              </tr>
            </template>
            <template v-else-if="jadwalList.length > 0">
              <tr v-for="item in jadwalList" :key="item.id" class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                <td class="px-6 py-4 font-medium text-slate-900 dark:text-white">
                  {{ item.periode?.nama }}
                </td>
                <td class="px-6 py-4">
                  <div class="flex flex-col">
                    <span class="font-medium text-slate-800 dark:text-slate-200">
                      {{ formatDateDisplay(item.tanggal) }}
                    </span>
                    <span class="text-xs text-slate-500">
                      {{ item.sesi?.jam_mulai }} - {{ item.sesi?.jam_selesai }} ({{ item.sesi?.nama }})
                    </span>
                  </div>
                </td>
                <td class="px-6 py-4">
                  <div class="flex items-center gap-2">
                    <UIcon name="i-heroicons-map-pin" class="text-slate-400" />
                    <span>{{ item.ruang?.nama }} ({{ item.ruang?.kode }})</span>
                  </div>
                </td>
                <td class="px-6 py-4 text-center">
                  <div class="flex flex-col items-center">
                    <span class="font-bold text-slate-900 dark:text-white text-lg">
                      {{ item.terisi }} <span class="text-slate-400 text-sm font-normal">/ {{ item.kuota }}</span>
                    </span>
                    <div class="w-16 h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full mt-1 overflow-hidden">
                      <div 
                        class="h-full bg-primary transition-all"
                        :style="{ width: Math.min((item.terisi / item.kuota) * 100, 100) + '%' }"
                      ></div>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-4 text-center">
                   <span 
                      v-if="item.is_active" 
                      class="px-2.5 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400"
                    >
                      Buka
                    </span>
                    <span 
                      v-else 
                      class="px-2.5 py-1 rounded-full text-xs font-semibold bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-500"
                    >
                      Tutup
                    </span>
                </td>
                <td class="px-6 py-4 text-right">
                  <div class="flex items-center justify-end gap-2">
                    <button 
                      @click="openEditModal(item)"
                      class="p-2 text-slate-400 hover:text-primary hover:bg-primary/5 rounded-lg transition-colors"
                      title="Edit"
                    >
                      <UIcon name="i-heroicons-pencil-square" class="text-lg" />
                    </button>
                    <button 
                      @click="confirmDelete(item)"
                      class="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"
                      title="Hapus"
                    >
                      <UIcon name="i-heroicons-trash" class="text-lg" />
                    </button>
                  </div>
                </td>
              </tr>
            </template>
            <tr v-else>
               <td colspan="6" class="px-6 py-12 text-center text-slate-400">
                 Tidak ada data jadwal ujian {{ selectedPeriodeId ? 'untuk periode terpilih' : '' }}
               </td>
            </tr>
          </tbody>
        </table>
      </div>
    </div>
    
    <!-- Modal Form -->
    <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" @click="closeModal"></div>
      <div class="bg-white dark:bg-surface-dark rounded-xl shadow-xl w-full max-w-md relative z-10 overflow-hidden flex flex-col max-h-[90vh]">
        <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
          <h3 class="text-lg font-bold text-slate-900 dark:text-white">
            {{ modalMode === 'create' ? 'Tambah Jadwal Ujian' : 'Edit Jadwal Ujian' }}
          </h3>
          <button @click="closeModal" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300">
            <UIcon name="i-heroicons-x-mark" class="text-xl" />
          </button>
        </div>
        
        <div class="p-6 overflow-y-auto">
          <form @submit.prevent="handleSubmit" class="space-y-4">
            <!-- Periode -->
            <div>
              <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Periode Pendaftaran</label>
              <select 
                v-model="formData.periode_id"
                class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary/50 text-slate-900 dark:text-white dark:bg-black/20"
                :class="errors.periode_id ? 'border-red-500 focus:border-red-500' : 'border-slate-300 dark:border-slate-700'"
              >
                <option value="0" disabled>Pilih Periode</option>
                <option v-for="p in periodeList" :key="p.id" :value="p.id">
                  {{ p.nama }}
                </option>
              </select>
              <p v-if="errors.periode_id" class="text-red-500 text-xs mt-1">{{ errors.periode_id[0] }}</p>
            </div>

            <!-- Tanggal -->
            <div>
              <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Tanggal Ujian</label>
              <input 
                v-model="formData.tanggal"
                type="date" 
                class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary/50 text-slate-900 dark:text-white dark:bg-black/20"
                :class="errors.tanggal ? 'border-red-500 focus:border-red-500' : 'border-slate-300 dark:border-slate-700'"
              />
              <p v-if="errors.tanggal" class="text-red-500 text-xs mt-1">{{ errors.tanggal[0] }}</p>
            </div>

            <div class="grid grid-cols-2 gap-4">
               <!-- Sesi -->
               <div>
                  <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Sesi / Waktu</label>
                  <select 
                    v-model="formData.sesi_id"
                    class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary/50 text-slate-900 dark:text-white dark:bg-black/20"
                    :class="errors.sesi_id ? 'border-red-500 focus:border-red-500' : 'border-slate-300 dark:border-slate-700'"
                  >
                    <option value="0" disabled>Pilih Sesi</option>
                    <option v-for="s in sesiList" :key="s.id" :value="s.id">
                      {{ s.nama }} ({{ s.jam_mulai }} - {{ s.jam_selesai }})
                    </option>
                  </select>
                  <p v-if="errors.sesi_id" class="text-red-500 text-xs mt-1">{{ errors.sesi_id[0] }}</p>
               </div>

               <!-- Ruang -->
               <div>
                  <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Ruang Ujian</label>
                  <select 
                    v-model="formData.ruang_id"
                    class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary/50 text-slate-900 dark:text-white dark:bg-black/20"
                    :class="errors.ruang_id ? 'border-red-500 focus:border-red-500' : 'border-slate-300 dark:border-slate-700'"
                  >
                    <option value="0" disabled>Pilih Ruang</option>
                    <option v-for="r in ruangList" :key="r.id" :value="r.id">
                      {{ r.nama }} (Kap: {{ r.kapasitas }})
                    </option>
                  </select>
                  <p v-if="errors.ruang_id" class="text-red-500 text-xs mt-1">{{ errors.ruang_id[0] }}</p>
               </div>
            </div>

            <!-- Kuota -->
            <div>
              <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Kuota Peserta</label>
              <input 
                v-model.number="formData.kuota"
                type="number" 
                min="1"
                class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary/50 text-slate-900 dark:text-white dark:bg-black/20"
                :class="errors.kuota ? 'border-red-500 focus:border-red-500' : 'border-slate-300 dark:border-slate-700'"
              />
              <p v-if="errors.kuota" class="text-red-500 text-xs mt-1">{{ errors.kuota[0] }}</p>
            </div>

            <!-- Status -->
            <div class="flex items-center gap-3 pt-2">
               <button 
                 type="button" 
                 role="switch" 
                 :aria-checked="formData.is_active"
                 @click="formData.is_active = !formData.is_active"
                 class="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2"
                 :class="formData.is_active ? 'bg-primary' : 'bg-slate-200 dark:bg-slate-700'"
               >
                 <span 
                   aria-hidden="true" 
                   class="pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out"
                   :class="formData.is_active ? 'translate-x-5' : 'translate-x-0'"
                 ></span>
               </button>
               <span class="text-sm text-slate-700 dark:text-slate-300">
                 {{ formData.is_active ? 'Dibuka untuk pemilih jadwal' : 'Ditutup sementara' }}
               </span>
            </div>
          </form>
        </div>

        <div class="px-6 py-4 bg-slate-50 dark:bg-black/20 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-3">
          <button 
            type="button" 
            @click="closeModal"
            class="px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 dark:bg-slate-800 dark:text-slate-200 dark:border-slate-600 dark:hover:bg-slate-700"
          >
            Batal
          </button>
          <button 
            type="button" 
            @click="handleSubmit"
            :disabled="isSubmitting"
            class="px-4 py-2 text-sm font-medium text-white bg-primary rounded-lg hover:bg-primary-600 disabled:opacity-70 disabled:cursor-wait flex items-center gap-2"
          >
            <UIcon v-if="isSubmitting" name="i-heroicons-arrow-path" class="animate-spin" />
            {{ modalMode === 'create' ? 'Simpan' : 'Update' }}
          </button>
        </div>
      </div>
    </div>

    <!-- Confirm Delete Modal -->
    <div v-if="deleteConfirmationOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" @click="deleteConfirmationOpen = false"></div>
      <div class="bg-white dark:bg-surface-dark rounded-xl shadow-xl w-full max-w-sm relative z-10 p-6 text-center">
        <div class="w-12 h-12 bg-red-100 text-red-600 rounded-full flex items-center justify-center mx-auto mb-4">
          <UIcon name="i-heroicons-exclamation-triangle" class="text-2xl" />
        </div>
        <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-2">Hapus Jadwal?</h3>
        <p class="text-sm text-slate-500 mb-6">
          Anda akan menghapus jadwal ujian ini. Tindakan ini tidak dapat dibatalkan.
        </p>
        <div class="flex gap-3 justify-center">
          <button 
            @click="deleteConfirmationOpen = false"
            class="px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 dark:bg-slate-800 dark:text-slate-200 dark:border-slate-600 dark:hover:bg-slate-700"
          >
            Batal
          </button>
          <button 
            @click="handleDelete"
            class="px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700"
          >
            Ya, Hapus
          </button>
        </div>
      </div>
    </div>

  </div>
</template>
